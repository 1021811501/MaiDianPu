    ...
    if (self.isRecoderSelectPicker){
        ...
        for (MLSelectPhotoAssets *asset in self.selectAsstes) {
            for (MLSelectPhotoAssets *asset2 in self.dataArray) {
                if ([asset.asset1.defaultRepresentation.url isEqual:asset2.asset1.defaultRepresentation.url]) {
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.item == 0 && self.topShowPhotoPicker){
        ...
        if (self.isRecoderSelectPicker) {
            ...
            {
                if ([asset isKindOfClass:[MLSelectPhotoAssets class]] && [asset.asset1.defaultRepresentation.url isEqual:[self.dataArray[indexPath.item] asset1].defaultRepresentation.url])
                {
